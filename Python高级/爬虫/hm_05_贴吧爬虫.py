...
class TeibaSpider:
    ...
    def extract_tiezi_url(self, html):
        ...
        tieba_url_list = html_obj.xpath('//a[@class="j_th_tit "]/@href')
        # 返回url
        return ["http://tieba.baidu.com/" + x for x in tieba_url_list]

    def extract_img_url(self, html):
        html_obj = etree.HTML(html)
        tieba_img_list = html_obj.xpath('//img[@class="BDE_Image"]/@src')
    ...
    def start(self, teiba_name, page_num):
        for page in range(1, page_num+1):
            # 写请求头
            query_dict = {"ie": "utf-8", "kw": teiba_name, "pn": (page - 1) * 50}
            url = "https://tieba.baidu.com/f?" + parse.urlencode(query_dict)
            # 获取贴吧响应代码
...
if __name__ == '__main__':
    ...
    spider = TeibaSpider()
    spider.start(teiba_name, page_num)
